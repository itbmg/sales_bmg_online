...
public partial class AgentWiseInveTransactions : System.Web.UI.Page
{
    MySqlCommand cmd;
    string UserName = "";
    VehicleDBMgr vdm;
            ...
            foreach (DataRow drbranch in dtbranch.Rows)
            {
                ...
                if (Routename != drbranch["RouteName"].ToString())
                {
                    if (Totalcount == 1)
                    {
                        ...
                        newRow["Route Sno"] = drbranch["RouteSno"].ToString();
