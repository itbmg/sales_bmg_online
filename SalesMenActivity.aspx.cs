...
public partial class SalesMenActivity : System.Web.UI.Page
{
    MySqlCommand cmd;
    string UserName = "";
    VehicleDBMgr vdm;
    ...
    void GetReport()
    {
        try
        {
            ...
            DataTable dttotShorts = new DataTable();
            ...
            if (dttable.Rows.Count > 0)
            {
                ...
                DataTable empltbl = dtview.ToTable(true, "EmpName", "EmpId");
                int i = 1;
                foreach (DataRow branch in empltbl.Rows)
                {
                    ...
                    routename["SNo"] = i;
                    routename["Route Name"] = branch["EmpName"].ToString();
                    ...
                    {
                        if (branch["EmpId"].ToString() == dr["EmpId"].ToString())
                        {
                            double TotalLeaks = 0;
                    ...
                    DataRow newshort = Report.NewRow();
                    newshort["Type"] = "Shortage";
                    double totalshort = 0;
                    double shortavg = 0;
                    foreach (DataRow dr in dtShorts.Rows)
                    {
                        if (branch["EmpId"].ToString() == dr["EmpId"].ToString())
                        {
                            double TotalLeaks = 0;
                            double.TryParse(dr["shortQty"].ToString(), out TotalLeaks);
                            double tot = TotalLeaks / 2;
                            newshort[dr["ProductName"].ToString()] = Math.Round(TotalLeaks, 2);
                            totalshort += TotalLeaks;
                        }
                    }
                    if (totdispatch != 0)
                    {
                        shortavg = (totalshort / totdispatch) * 100;
                    }
                    newshort["Total ltrs"] = Math.Round(totalshort, 2);
                    newshort["Average For Total Dispatch"] = Math.Round(shortavg, 2);
                    if (totdispatch != 0)
                    {
                        Report.Rows.Add(newshort);
                    }
                ...
                ////DataRow brk1 = Report.NewRow();
                ////brk1["Route Name"] = "";
                ////Report.Rows.Add(brk1);
                ////DataRow newrowTotalDispatch = Report.NewRow();
                ////newrowTotalDispatch["Route Name"] = "Dispatch";
                ...
                //}
                //DataRow newShorttotal = Report.NewRow();
                //newShorttotal["Type"] = "Shortage";
                //double totshortavg = 0;
                //double allShortages = 0;
                //foreach (DataRow dr in dttotShorts.Rows)
                //{

                //    double TotalLeaks = 0;
                //    double.TryParse(dr["shortQty"].ToString(), out TotalLeaks);
                //    double tot = TotalLeaks / 2;
                //    newShorttotal[dr["ProductName"].ToString()] = Math.Round(TotalLeaks, 2);
                //    allShortages += TotalLeaks;
                //}
                //if (totaldispatch != 0)
                //{
                //    totshortavg = (allShortages / totaldispatch) * 100;
                //}
                //newShorttotal["Total ltrs"] = Math.Round(allShortages, 2);
                //newShorttotal["Average For Total Dispatch"] = Math.Round(totshortavg, 2);
                //if (totaldispatch != 0)
                //{
                //    Report.Rows.Add(newShorttotal);
                //}
