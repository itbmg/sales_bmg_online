...
public partial class dcvssale : System.Web.UI.Page
{
    ...
    void GetReport()
    {
        try
        {
            ...
            //cmd = new MySqlCommand("SELECT TripInfo.Sno, ROUND(SUM(ProductInfo.Qty), 2) AS dispatchqty, TripInfo.BranchName, TripInfo.Branch_Id, TripInfo.DispName, TripInfo.BranchID FROM            (SELECT        tripdata.Sno, tripdata.AssignDate, tripdata.I_Date, branchdata_1.BranchName, dispatch.BranchID, dispatch.Branch_Id, dispatch.GroupId, dispatch.CompanyId, dispatch.DispName    FROM            branchdata INNER JOIN    dispatch ON branchdata.sno = dispatch.Branch_Id INNER JOIN      triproutes ON dispatch.sno = triproutes.RouteID INNER JOIN     tripdata ON triproutes.Tripdata_sno = tripdata.Sno INNER JOIN     branchdata branchdata_1 ON dispatch.Branch_Id = branchdata_1.sno   WHERE        (dispatch.Branch_Id = @BranchID) AND (tripdata.AssignDate BETWEEN @d1 AND @d2)) TripInfo INNER JOIN   (SELECT        Sno, Qty    FROM            (SELECT        tripdata_1.Sno, tripsubdata.Qty     FROM            tripdata tripdata_1 INNER JOIN  tripsubdata ON tripdata_1.Sno = tripsubdata.Tripdata_sno    WHERE        (tripdata_1.AssignDate BETWEEN @d1 AND @d2)) TripSubInfo) ProductInfo ON TripInfo.Sno = ProductInfo.Sno GROUP BY TripInfo.DispName, TripInfo.BranchID ORDER BY TripInfo.AssignDate");
            ...
            if (dtDispatchesbranches.Rows.Count > 0)
            {
                ...
                Report.Columns.Add("DiffMilk").DataType = typeof(Double); ;
                Report.Columns.Add("TotalLeaks").DataType = typeof(Double); ;
                Report.Columns.Add("VTotalLeaks").DataType = typeof(Double); ;
                Report.Columns.Add("DiffLeaks").DataType = typeof(Double); ;
                ...
                foreach (DataRow branch in distincttable.Rows)
                {
                    ...
                    foreach (DataRow drrdelivery in dtDispatchesbranches.Select("BranchID='" + BRANCHID + "'AND I_Date='" + branch["I_date"].ToString() + "'"))
                    {
                        ...
                        DispQty += dcqty;
                        newrow["TotalDispatch"] = Math.Round(DispQty, 0);
                    }
                    foreach (DataRow drrdelivery in dtAgent.Select("SuperBranch='" + BRANCHID + "'AND IndentDate='" + branch["I_date"].ToString() + "'"))
                    {
                        ...
                        newrow["TotalSale"] = Math.Round(saleqty, 0); //drrdelivery["DeliveryQty"].ToString();
                    }
                    foreach (DataRow drleaks in DtLeksAndReturns.Select("BranchID='" + BRANCHID + "'AND AssignDate='" + branch["I_date"].ToString() + "'"))
                    {
                        double.TryParse(drleaks["vleaks"].ToString(), out Vleakqty);
                        double.TryParse(drleaks["totleaks"].ToString(), out leakqty);
                        double.TryParse(drleaks["vreturns"].ToString(), out returnqty);
                        newrow["TotalLeaks"] = Math.Round(leakqty, 0); //drleaks["Leaks"].ToString();
                        newrow["VTotalLeaks"] = Math.Round(Vleakqty, 0); //drleaks["Leaks"].ToString();
                        newrow["TotalReturns"] = Math.Round(returnqty, 0); //drleaks["Return"].ToString();
                        totalleakreturn += leakqty;
                        totalleakreturn += returnqty;
                    }
                    foreach (DataRow drfree in dtShortAndFree.Select("SubBranch='" + BRANCHID + "'AND I_date='" + branch["I_date"].ToString() + "'"))
                    {
                        double.TryParse(drfree["ShortQty"].ToString(), out shortqty);
                        double.TryParse(drfree["FreeMilk"].ToString(), out freeqty);
                        newrow["TotalFree"] = Math.Round(freeqty, 0); //drfree["ShortQty"].ToString();
                        newrow["TotalShort"] = Math.Round(shortqty, 0);// drfree["FreeMilk"].ToString();
                    ...
                    double diffqty = DispQty - saleqty;
                    double diffleak = leakqty - Vleakqty;

                    newrow["DiffMilk"] = Math.Round(diffqty, 0);
                    newrow["DiffLeaks"] = Math.Round(diffleak, 0);
