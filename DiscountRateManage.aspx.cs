...
public partial class DsicountRateManage : System.Web.UI.Page
{
    MySqlCommand cmd;
    string BranchID = "";
    VehicleDBMgr vdm;
        ...
        try
        {
            ...
            if (dtBranch.Rows.Count > 0)
            {
                foreach (DataRow dr in dtBranch.Rows)
                {
                    ...
                    drnew["discountprice"] = dr["discountprice"].ToString();
                    ...
                    foreach (DataRow dr in dtAgents.Rows)
                    {
                        try
                        {
                            if (branch["BranchName"].ToString() == dr["BranchName"].ToString())
                            {
                                ...
                                newrow[dr["ProductName"].ToString()] = discountprice;
                            }
                        }
                        catch
                        {
    ...
    protected void btn_WIDB_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            DateTime ServerDateCurrentdate = VehicleDBMgr.GetTime(vdm.conn);
            DataTable dt = (DataTable)Session["dtImport"];
            cmd = new MySqlCommand("SELECT branch_sno, product_sno, discountprice FROM branchproducts  ");
            DataTable dtBrnchPrdt = vdm.SelectQuery(cmd).Tables[0];
            int i = 0;
            foreach (DataRow dr in dt.Rows)
            {
                ...
                dtAgentprdt.Columns.Add("branch_sno");
                dtAgentprdt.Columns.Add("product_sno");
                dtAgentprdt.Columns.Add("discountprice");
                DataRow[] drBp = dtBrnchPrdt.Select("branch_sno='" + dr["Agent Code"].ToString() + "'");
                for (int k = 0; k < drBp.Length; k++)
                {
                    DataRow newrow = dtAgentprdt.NewRow();
                    ...
                    newrow["discountprice"] = drBp[k][2].ToString();
                    dtAgentprdt.Rows.Add(newrow);
                }
                int j = 3;
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    {
                        string discountprice = dt.Rows[i][j].ToString();
                        if (discountprice == "&nbsp;")
                        {
                            discountprice = "0";
                        }
                        ...
                        DataTable oldunitprice = new DataTable();
                        oldunitprice.Columns.Add("discountprice");
                        DataRow[] drAp = dtAgentprdt.Select("product_sno='" + ProductID + "'");
                        if (drAp.Length == 0)
                        {
                            if (discountprice == "0")
                            ...
                            {
                                cmd = new MySqlCommand("insert into branchproducts (branch_sno,product_sno,discountprice,userdata_sno,DTarget,WTarget,MTarget) values (@branchname,@productname,@discountprice, @username,@DTarget,@WTarget,@MTarget)");
                                cmd.Parameters.AddWithValue("@branchname", AgentCode);
                                cmd.Parameters.AddWithValue("@productname", ProductID);
                                float DiscountUntCost = 0;
                                float.TryParse(discountprice, out DiscountUntCost);
                                cmd.Parameters.AddWithValue("@discountprice", DiscountUntCost);
                                cmd.Parameters.AddWithValue("@username", Session["userdata_sno"]);
                                int productDaytarget = 0;
                                int productWeektarget = 0;
                                int productMonthtarget = 0;
                        ...
                        else
                        {
                            ...
                            {
                                DataRow newaprow = oldunitprice.NewRow();
                                newaprow["discountprice"] = drAp[ap][2].ToString();
                                oldunitprice.Rows.Add(newaprow);
                            }
                            string oldprice = "0";
                            ...
                            if (oldunitprice.Rows.Count > 0)
                            {
                                oldprice = oldunitprice.Rows[0]["discountprice"].ToString();
                            }
                            float discountUnitCost = 0;
                            float.TryParse(discountprice, out discountUnitCost);
                            float oldDiscountUnitCost = 0;
                            float.TryParse(oldprice, out oldDiscountUnitCost);
                            if (discountUnitCost == oldDiscountUnitCost)
                            {

                            }
                            else
                            {
                                cmd = new MySqlCommand("Update branchproducts set discountprice=@discountprice where Branch_sno=@Branch_sno and Product_sno=@Product_sno");
                                cmd.Parameters.AddWithValue("@discountprice", discountprice);
                                cmd.Parameters.AddWithValue("@Branch_sno", AgentCode);
                                cmd.Parameters.AddWithValue("@Product_sno", ProductID);
                                vdm.Update(cmd);
                                cmd = new MySqlCommand("insert into productsrateslogs (PrdtSno,BranchId,discountprice,editeddiscountprice,EditedUserid,DateOfEdit) values (@PrdtSno,@BranchId,@discountprice,@editeddiscountprice,@EditedUserid,@DateOfEdit)");
                                cmd.Parameters.AddWithValue("@PrdtSno", ProductID);
                                cmd.Parameters.AddWithValue("@BranchId", AgentCode);
                                cmd.Parameters.AddWithValue("@discountprice", oldDiscountUnitCost);
